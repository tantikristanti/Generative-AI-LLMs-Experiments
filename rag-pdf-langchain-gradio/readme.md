...
1. To run the application using Gradio interface:
...
2. To run the application using Streamlit:

http://localhost:8501/

The running application is listening on localhost, port 7860: **http://localhost:8501**
...
The experimental outcomes across diverse scenarios indicate that processing time is significantly influenced by data volume, embedding model selection, and LLM architecture. For instance, using the 'deepseek-r1' embedding model paired with an LLM resulted in a 15-minute runtime, whereas employing the 'nomic-embed-text' embedding module in tandem with a LLM architecture led to a faster processing time of 10 minutes.
